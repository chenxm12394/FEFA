...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, src, flow, interp=False):
        """
        Push the src and flow through the spatial transform block
            :param src: the original moving image
            :param flow: the output from the U-Net
        """
        # theta = torch.Tensor([[[1,0,0],[0,1,0]]]).to(src, non_blocking=True)
        # grid = torch.nn.functional.affine_grid( \
        #         theta, src[0:1].shape, align_corners=False)
        # grid = grid + flow.permute(0, 2, 3, 1)
        # warped = torch.nn.functional.grid_sample( \
        #         src.float(), grid.float(), align_corners=False)
        
        # if interp and (warped.shape != src.shape):
        #     warped = torch.nn.functional.interpolate( \
        #             warped, size=src.shape[2:])
        ...
        print(torch.min(new_locs), torch.max(new_locs))
        if len(shape) == 2:
