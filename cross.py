...
class SpatialTransformer(torch.nn.Module):
    def __init__(self, channels=1):
        ...
        self.net = torch.nn.Sequential( \
                UNet(2*channels, 32, (16, 32, 32, 64, 64)), \
                # UNet(2*channels, 32, (8, 16, 32, 64, 64)), \
                torch.nn.LeakyReLU(inplace=True), \
                torch.nn.Conv2d(32, 2, kernel_size=3, padding=1))
        # # self.net = DeformableNet(to_2tuple(shape))
        # self.net = PCNet()
        with torch.no_grad():
    ...
    def forward(self, moving, fixed, features=None):
        ...
        offset = \
                self.net(torch.cat([moving, fixed], dim=1)).permute(0, 2, 3, 1)
        grid = grid + offset
        return offset, grid
